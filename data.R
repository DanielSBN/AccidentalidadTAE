...
comunas <- function(train) {
  ...
  datos[['Belen']] <- as.data.frame(subset(train[(train$COMUNA == 'Bel̩n'),], select=-c(ANO, COMUNA)))
  ...
  datos[['La America']] <- as.data.frame(subset(train[(train$COMUNA == 'La Am̩rica'),], select=-c(ANO, COMUNA)))
